from docBot import PriaidDiagnosisClient
import random
...
class PriaidDiagnosisClientDemo:
    ...
    def loadDiagnosis(self, selectedSymptoms,gender,dob):
        self._writeHeaderMessage("Diagnosis")
            
        if( gender == "female"):
            paramGender = PriaidDiagnosisClient.Gender.Female
        else:
            paramGender= PriaidDiagnosisClient.Gender.Male

        diagnosis = self._diagnosisClient.loadDiagnosis(selectedSymptoms, paramGender, dob)
        if diagnosis:
            return diagnosis
    ...
    def loadSpecialisations(self, selectedSymptoms):
    ...
    def loadRedFlag(self, selectedSymptom):
    ...
    def loadProposedSymptoms(self, selectedSymptoms):
        selectedSymptomsIds = []
        for symptom in selectedSymptoms:
            selectedSymptomsIds.append(symptom["ID"])
    ...
    def loadSymptoms(self):
        return self._diagnosisClient.loadSymptoms()
    def loadIssues(self):
        return self._diagnosisClient.loadIssues()
